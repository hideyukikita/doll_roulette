...
- `doll_image_url`: VARCHAR(255) (NULL可) — 当選時に表示した画像URL（代表画像・サブ画像を合わせた中からランダムで選んだ1枚を保存）
- `selected_at`: TIMESTAMP (Default: now)


## 5. 開発履歴（参考）

環境構築 → DB・API → フロント → ルーレット → 履歴・リセット → 写真複数・お出かけ日記 の順で実装済み。

## 6. 将来の拡張（任意）
### 二次開発フェーズ（一部実装済み）
- **写真登録機能**: ✅ 実装済み。登録・編集時に画像アップロード（家族は複数枚・追加/削除）、当選時・履歴に表示。家族詳細で代表画像・サブ画像をタップすると画像全体を表示。**画像全体表示時にダウンロードボタンで端末に保存可能**。
- **お出かけ日記**: ✅ 実装済み。場所・日付・一緒に・写真（複数）・コメント。一覧の行タップで**手前にオーバーレイ**で詳細表示。編集・削除・詳細で画像タップで全体表示。写真削除時に確認ダイアログ。
- **ルーレット当選画像**: ✅ 代表画像（dolls.image_url）・サブ画像（doll_images）を合わせた中からランダム1枚を表示し、そのURLを履歴に保存。
- **当選履歴の詳細**: ✅ 当選履歴の行をタップすると手前にオーバーレイで詳細表示。詳細で画像をタップすると画像全体を表示。削除は詳細内から。
- **モバイル最適化（将来）**: スマホで使いやすくするためのUI調整。API向き先は同一ホストで自動判定済み。具体的な候補は下記「スマホ用UI調整の候補」を参照。
#### スマホ用UI調整の候補（モバイル最適化で想定する内容）
- **タップ領域**: ボタン・リスト行を指で押しやすいサイズに（最低 44px 程度）
- **フォント・余白**: 小さい画面でも読みやすいフォントサイズと余白
- **ナビ・ヘッダ**: 画面幅が狭いときのナビゲーション（ハンバーガーメニュー等）
- **画像・グリッド**: 小画面での写真枚数やアスペクト比の調整
- **PWA / ホーム画面追加**: ブラウザから「ホームに追加」でアプリのように起動（オプション）

※ 現状もスマホから利用可能。上記は「より使いやすくする」ための追加検討項目。
