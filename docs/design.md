# ぬいぐるみルーレット開発計画書（WSL2/Docker構成）

## 1. プロジェクト概要
- **目的**: 飼っているぬいぐるみを平等に選び、一緒に寝る子を楽しく決める。
- **ターゲット環境**: 
...
### A. ぬいぐるみ管理
- **登録機能**: 名前と色を選択して登録。
- **一覧表示**: 登録済みのぬいぐるみを確認、不要な子の削除・新しい子の追加。
### B. ルーレット機能
- **演出**: 選ばれていない子の中からランダムに回転・決定する楽しい演出。
- **ロジック (パターンB)**: 
    - 一度選ばれた子は `is_selected: true` となり、次回の対象から外れる(めちゃめちゃ確率低い状態に)。
    - 全員が選ばれたら「全員一周したよ！」と表示し、ルーレットを停止する。
- **UI表示**: 「残り○匹 / 全○匹」のカウンター表示、当選履歴（日時付き）。
### C. 履歴・リセット
- **手動リセット**: リセットボタン押下で、全ぬいぐるみの `is_selected` を `false` に戻す。リセット時には間違い防止の確認。
...
- **Step 2**: DB接続設定 & API作成 (ぬいぐるみ一覧・登録)
...
## 6. 将来の拡張ロードマップ
### 二次開発フェーズ
- **写真登録機能**: 写真のアップロード・表示（Docker Volume管理）。
- **モバイル最適化**: スマホ用UI調整、PCスリープ対策。
### 三次開発フェーズ
