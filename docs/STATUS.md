...
- **一覧**: 登録済みのかぞくたちを表示（サムネイルは image_urls[0] または image_url）。画像更新後はキャッシュバスターで再表示
- **編集**: 名前・色・画像の変更。**複数画像**の追加（POST `/api/dolls/:id/images`）・削除（POST `/api/dolls/:id/images/remove`、JSON body）
- **削除**: DELETE `/api/dolls/:id`
...
- **一覧・詳細**: 一覧で概要表示、タップで詳細。詳細で「一緒に」を写真の上に表示（名前はテーマカラー反映）、複数画像はグリッド表示
- **編集**: 場所・日付・一緒に・コメントを PUT `/api/outings/:id` で更新。写真は**追加**（POST `/api/outings/:id/images`）・**削除**（POST `/api/outings/:id/images/remove`、JSON body）
...
- **当選表示**: 回転停止後、結果を3秒表示。**登録画像のうちランダムで1枚**を表示（複数画像の場合は doll_images から1枚、なければ dolls.image_url）
...
- 当選履歴1件削除: DELETE `/api/histories/:id`（その子はルーレットに復活）
...
4. かぞく一覧で登録（画像・複数画像）→ 編集（写真追加・削除）・削除を確認
5. お出かけ日記で登録・編集・写真追加・削除・詳細で画像タップで全体表示を確認
6. ルーレットで回す → 当選表示（登録画像からランダム1枚）・当選履歴（その時の画像）・リセットを確認
7. スマホから接続する場合は README.md の「スマホからの接続方法」を参照（API 向き先は自動判定も可）
