...
- **詳細**: 一覧の行をタップすると手前にオーバーレイで詳細を表示。名前・色・**代表画像**（1枚）・**サブ画像**（複数）を表示し、ここから編集・削除
...
4. かぞく一覧で登録（画像・複数画像）→ 行タップで詳細表示（代表・サブ画像）→ 詳細から編集（代表・サブの差し替え・追加・削除）・削除を確認
5. お出かけ日記で登録・編集・写真追加・削除・一覧の行タップで詳細オーバーレイ・詳細で画像タップで全体表示を確認
