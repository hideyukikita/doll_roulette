/**
 * 円盤ルーレット（扇形セグメント・針・回転）
 * 当選者が12時の位置に来るように停止する
 */
import { useEffect, useState, useRef } from "react";
import type { Doll } from "../types/doll.js";
import { getDollFillColor, getDollSegmentTextColor } from "../utils/colors.js";
...
export default function RouletteWheel({ dolls, winnerId, spinning, onSpinComplete }: RouletteWheelProps) {
  ...
  if (dolls.length === 0) {
    return (
      <div className="flex items-center justify-center h-[320px] text-stone-400">
        回すと選ばれるよ
      </div>
    );
  }
  ...
  return (
    <div className="relative inline-block my-6">
      ...
      />
      <div
        className="absolute z-20 w-5 h-5 rounded-full bg-pink-400 border-2 border-white"
        ...
        >
          <g transform={`translate(${WHEEL_SIZE / 2}, ${WHEEL_SIZE / 2})`}>
            {dolls.map((doll, i) => {
              ...
              const textRotation =
                dolls.length === 1 ? 0 : (i * segmentAngle + segmentAngle / 2 - 90 + (labelY > 0 ? 180 : 0));
